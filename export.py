...
def quantize_q80_ng(w):
    """
    takes a tensor and returns the Q8_0 quantized version
    i.e. symmetric quantization into int8, range [-127,127]
    """
    """
    Add your quantization code here
    """
    maxerr = torch.abs(fp32val - w.flatten()).max().item()
    return int8val, scale, maxerr

def quantize_q80_g(w, group_size):
    """
    takes a tensor and returns the Q8_0 quantized version
    i.e. symmetric quantization into int8, range [-127,127]
    """
    ...
    """
    Add your grouped quantization code here, you can refer to
    https://github.com/karpathy/llama2.c for inspiration
    """
...
def version2_export(model, filepath):
    ...
    - quantization is done in no groups
    """
    version = 2
    ...
    for i, w in enumerate(weights):
        ...
        q, s, err = quantize_q80_ng(w)
        # save the int8 weights to file
        serialize_int8(out_file, q) # save the tensor in int8
        #serialize_fp32(out_file, s) # save scale factors
        out_file.write(struct.pack('f', s)) # save scale factors
        # logging
...
def version3_export(model, filepath, group_size=16):
    """
    Export the model weights in Q8_0 into .bin file to be read from C.
    That is:
    - quantize all weights to symmetric int8, in range [-127, 127]
    ...
    for i, w in enumerate(weights):
        # quantize this weight
        q, s, err = quantize_q80_g(w, group_size)
        # save the int8 weights to file
        serialize_int8(out_file, q) # save the tensor in int8
        serialize_fp32(out_file, s) # save scale factors
        # logging
...
def model_export(model, filepath, version, dtype=torch.float32):
    ...
    v1: float32 export
    v2: int8 quantized Q8_0 export, no groups
    v3: int8 quantized Q8_0 export, similar to llama.cpp, in groups
